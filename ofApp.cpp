...
void ofApp::update() {
	...
	for (int i = 0; i < this->triangle_list.size(); i++) {
		...
		auto noise_value = ofNoise(avg.x * 0.005 + ofGetFrameNum() * 0.008, avg.y * 0.005, avg.z * 0.005);
...
void ofApp::draw() {
	...
	this->cam.begin();
	this->cam.setPosition(glm::vec3(0, 0, 200));
	ofRotateY(ofGetFrameNum() * 0.2);
	ofRotateX(sin(ofGetFrameNum() * 0.01) * 30);

	ofSetColor(128, 0,255, 32);
	this->mesh.drawFaces();
